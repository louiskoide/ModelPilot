...
Proxy tests bind local loopback sockets; a sandbox denial is not a product test failure. Last verification: 179 offline tests on September 22, 2026 (the earlier 143, plus 7 governed-proxy, 2 enforce/reconcile, 7 fallback, 2 cascade-harness, 17 hook and 1 offline governed-session; the governed session needs the `claude` CLI and is skipped in CI). On this Mac only system Python 3.9 exists; there 177 pass and the two `test_transport` HTTPError tests error because of a 3.9 `HTTPError` quirk, not a product failure. CI runs 3.10 and 3.14. No need to rerun expensive cache/TTL tests for documentation changes.
...
3. Plan in `docs/m6-benchmark-plan.md` (tasks mined from real repositories, SWE-bench style, split by repository into tuning/final; phases 3a–3d, first 3a and 3b at $0). Build representative, reproducible repository-task fixtures and shared graders; isolate checkouts and normalize tool access, model availability, auth, cache state and effort. Separate tuning from final evaluation.
