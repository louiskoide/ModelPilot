...
- `bench_tasks.py`: mines SWE-bench-style tasks from cloned repos (ignored `work/bench/repos/`), history-free checkouts, hidden grader (test dir restored to the reference commit, run without credentials), validation (`python3 -m modelpilot.bench_tasks validate`). Task specs are in `bench/tasks/`. `bench.py`: task × arm × trial harness with isolation, identical limits, wire accounting and grading. Fixed arms only for now; `--live` is billable.
...
Proxy tests bind local loopback sockets; a sandbox denial is not a product test failure. Last verification: 198 offline tests on September 22, 2026 (the earlier 143, plus 7 governed-proxy, 2 enforce/reconcile, 7 fallback, 2 cascade-harness, 17 hook, 1 offline governed-session, 14 bench-task/split and 5 bench-harness; the governed session and one bench trial test need the `claude` CLI and are skipped in CI). On this Mac only system Python 3.9 exists; there 196 pass and the two `test_transport` HTTPError tests error because of a 3.9 `HTTPError` quirk, not a product failure. CI runs 3.10 and 3.14. No need to rerun expensive cache/TTL tests for documentation changes.
