...
| M6 | Paired four-arm direct API smoke benchmark; 16/16 passed. Repository-task harness with fixed arms (live pilot passed) and both Jev arms (offline only, stub router) | Integrated ModelPilot arm; live Jev trials; tuning and final runs with repeated trials |
...
- `jev_accounted_launch.mjs`: harness launcher that puts ModelPilot's proxy behind Jev's for wire accounting. Its differences from the stock launcher are listed in its header; `--self-test` checks the plumbing without Claude Code; `--serve` runs only Jev's proxy for a bench trial (the harness starts Claude Code); `--stub-route` replaces TypeSafe in offline tests only.
...
- `bench_tasks.py`: mines SWE-bench-style tasks from cloned repos (ignored `work/bench/repos/`), history-free checkouts, hidden grader (test dir restored to the reference commit, run without credentials, fresh HOME/TMPDIR, skips never count as run), validation (`python3 -m modelpilot.bench_tasks validate`). Task specs are in `bench/tasks/`. `bench.py`: task × arm × trial harness with isolation, identical limits, wire accounting and grading. It adds a $0 reference preflight (a trial must pass as many hidden tests as its reference), a `single` or `followup` shape (`--resume` after `--gap`, other trials run during the gap), a pinned client binary, session stop reasons, leftover-process cleanup, and crash-safe records. Fixed and Jev arms run (ModelPilot arm refuses); `--live` is billable and requires `--run-budget` (and a TypeSafe key at a hidden prompt for Jev arms). One ModelPilot proxy serves a whole trial; `ProxyServer.wait_idle` makes sure every accepted request is logged at session end and close. `bench_jev.py`: per-trial Jev router (`jev_accounted_launch.mjs --serve`, Jev's proxy alive across a follow-up so its routing state carries), checkout verification before every session, key separation (router never gets the Anthropic key, client never the TypeSafe key), per-trial `routing` record, `jev_router_unavailable` run stop on a TypeSafe auth failure. Jev dollars are provider cost only (`cost_scope`, router unpriced, a lower bound). Rejected requests keep the headline cost unknown; `…_if_rejected_free_usd` is a labeled sensitivity figure. `bench_report.py`: cold-equivalent cost (cache reads inherited from earlier trials repriced as writes; measured cost alongside) and a paired task-level bootstrap; `python3 -m modelpilot.bench_report <run>` rebuilds a summary.
...
Proxy tests bind local loopback sockets; a sandbox denial is not a product test failure. Last verification: 281 offline tests on September 24, 2026 (the earlier 244 plus 37 for the Jev arms; the governed session, 5 offline bench trial tests and 3 offline Jev trial tests drive the real `claude` CLI, and the Jev launcher/trial tests need node plus both `work/jev-router-*` checkouts; all are skipped in CI). On this Mac, system Python 3.9 passes 279, and the two `test_transport` HTTPError tests error because of a 3.9 `HTTPError` quirk, not a product failure. Homebrew Python 3.12 (`python3.12`) passes all 281. CI runs 3.10 and 3.14. No need to rerun expensive cache/TTL tests for documentation changes.
...
1. Jev baseline. Credential and scoring gate passed at the stock deadline (`runs/jev-preflight-20260922-120150`, 393 ms). The pinned checkout was re-cloned into `work/jev-router-baseline` (65/65 upstream tests). The end-to-end check (`runs/jev-route-20260922-121832`) failed for two reasons. The Anthropic key was rejected (401). Separately, pinned Jev never routes Claude Code 2.1.278+, because those clients append a trailing `system` message and `newTurnPrompt()` requires a trailing `user` message; it silently pins opus. `python3 -m modelpilot.jev_compat --claude <cli>` checks this offline, and 2.1.101 is compatible. The user chose a separately labeled **compatibility-patched Jev** (`patches/jev-trailing-system-message.patch`, one functional line, built into `work/jev-router-compat` by `python3 -m modelpilot.jev_compat --prepare-compat`) on current Claude Code. The finding that stock Jev does not route is reported alongside it. Offline probe: the patched variant routes on 2.1.280, with the follow-up request on the same model. The wire-accounting adapter is built and passes offline: `configs/jev-rates.json`, proxy chunked uploads and `/v1/models` pass-through, `jev_accounted_launch.mjs`, and `reconcile()`. Live runs `jev-route-compat-20260922-133343` and `jev-route-compat-wire-20260922-144020` routed for real (TypeSafe opus->haiku, p=0.99), and ModelPilot's proxy saw the routed traffic. Both failed on Anthropic 401 (invalid key). The route check now pre-checks the key for free and sets `CLAUDE_CODE_MAX_RETRIES=0`, because retries re-triggered Jev routing 11×. The first successful live routed task is `runs/jev-route-compat-wire-20260922-151453`: opus->haiku, tokens reconcile exactly, $0.006426 provider cost. Claude Code misprices the sentinel roughly 4×, so client dollars are never used for Jev. One 400 occurred because Haiku rejects the system-role message; Claude Code resends, and Jev routes a second time. The user decided to **measure, not patch** this overhead: keep the one-line compat patch, and report `rejected_requests`, `extra_decisions` and router usage per task. The Jev arm's mechanics are validated. Both Jev arms now run in the bench harness (offline-verified September 24; see `docs/m6-benchmark-plan.md`, "Jev arms"). Claude Code 2.1.281 prices the sentinel at 4× Haiku / 2× Sonnet 5 / 0.8× Opus 5 wire cost, so a Jev session's client stop threshold trips at different real spend (recorded in the manifest). Remaining Jev gaps are TypeSafe pricing and confirming whether rejected 400s are billed; until then Jev dollars are provider-only lower bounds and rejected-request trials keep unknown headline cost. Do not count fail-open Claude (or stub-router offline decisions) as Jev routing.
2. Integration surface. Done offline in `governor.py`: durable reserve/settle with crash recovery, ledger-fenced review, rebase plan/ack, journal, governed worker transport, and multi-process tests. Wiring done offline on branch `governor-wiring` (see `docs/governor.md`, "Wiring"): proxy settlement through shared `mp-` request IDs, Claude Code hooks (observe, correction delivery, user-only rebase and stuck notices), and gated fallback execution. The first live session (`runs/governed-session-20260922-214046`) failed: accounting was exact, but the model rejected the hook-delivered correction as prompt injection. Fixed offline with a user-declared, coded correction channel: the launch prompt declares it, and hooks deliver only under its marker (see `docs/governor.md`). The second live run (`runs/governed-session-20260922-222741`) passed. Remaining: detection of `/model` and `/effort` rebuilds, and turn-end correction delivery. Keep dry-run until item 4 has evidence.
3. Plan in `docs/m6-benchmark-plan.md` (tasks mined from real repositories, SWE-bench style, split by repository into tuning/final). 3a and 3b done offline: 5 validated tasks (more-itertools, tomli) and the fixed-arm harness. The 3c live pilot passed (`runs/bench-20260923-070044`); its two warm starts show why costs are reported cold-equivalent. 3e is done: harness fixes found in an audit before any tuning run (see the plan's 3e table). 3d is done: 40 validated tasks from 8 repositories on Python 3.12 (`work/bench/py312`, pinned in `bench/environment.json`). The tuning set is more-itertools, tomli, cachetools and parse; the final set is click, packaging, boltons and humanize, hash-locked in `bench/splits.json`. Never edit a final task spec after the lock. Lock the venv's site-packages read-only before live runs. Build representative, reproducible repository-task fixtures and shared graders; isolate checkouts and normalize tool access, model availability, auth, cache state and effort. Separate tuning from final evaluation.
4. The ModelPilot arm's behavior is proposed in `docs/m6-modelpilot-policy.md`. It needs the user's approval for active mode in the benchmark arm only, and an M0 replication on Haiku 4.5, Sonnet 5 and Opus 5 first. Fixed and both Jev arms are runnable (Jev offline-verified only; run `jev_check --live` before a live Jev run). Add the integrated ModelPilot arm. Record any model-constrained Jev variant separately. Measure all costs, quality, wall time, routing failures and uncertainty together. Existing direct-API toy results cannot be reused as a fair agent baseline.
