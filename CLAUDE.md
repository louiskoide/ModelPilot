...
- `proxy.py`, `fixtures.py`, `soak.py`: streaming pass-through proxy and local synthetic checks. Proxy requests identity encoding to keep usage parsing reliable. No active routing.
...
Proxy tests bind local loopback sockets; a sandbox denial is not a product test failure. Last verification: 118 offline tests on September 22, 2026 (87 prior + 20 governor + 11 Jev route check). On this Mac only system Python 3.9 exists; there 116 pass and the two `test_transport` HTTPError tests error because of a 3.9 `HTTPError` quirk, not a product failure. CI runs 3.10 and 3.14. No need to rerun expensive cache/TTL tests for documentation changes.
