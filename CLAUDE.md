...
| M2 | SQLite versioned task ledger, leases/corrections, stuck ladder, output references and read-only MCP | Automatic tool-output interception; real mid-flight client correction delivery |
...
- `proxy.py`, `fixtures.py`, `soak.py`: streaming pass-through proxy and local synthetic checks. Proxy requests identity encoding to keep usage parsing reliable. No active routing. Accepts strict chunked uploads, passes through `GET /v1/models` only, and labels rows `kind`/`tool_count`.
...
Proxy tests bind local loopback sockets; a sandbox denial is not a product test failure. Last verification: 138 offline tests on September 22, 2026 (87 prior + 20 governor + 22 Jev route/wire/key + 5 proxy + 3 rates + 1 launcher self-test, skipped without `work/`). On this Mac only system Python 3.9 exists; there 136 pass and the two `test_transport` HTTPError tests error because of a 3.9 `HTTPError` quirk, not a product failure. CI runs 3.10 and 3.14. No need to rerun expensive cache/TTL tests for documentation changes.
