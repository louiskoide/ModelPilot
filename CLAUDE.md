...
Proxy tests bind local loopback sockets; a sandbox denial is not a product test failure. Last verification: 143 offline tests on September 22, 2026 (87 prior + 20 governor + 26 Jev route/wire/key + 6 proxy + 3 rates + 1 launcher self-test, skipped without `work/`). On this Mac only system Python 3.9 exists; there 141 pass and the two `test_transport` HTTPError tests error because of a 3.9 `HTTPError` quirk, not a product failure. CI runs 3.10 and 3.14. No need to rerun expensive cache/TTL tests for documentation changes.
