...
Proxy tests bind local loopback sockets; a sandbox denial is not a product test failure. Last verification: 107 offline tests on September 22, 2026 (87 prior + 20 governor). On the handoff Mac only system Python 3.9 exists; there 105 pass and the two `test_transport` HTTPError tests error because of a 3.9 `HTTPError` quirk, not a product failure. CI runs 3.10 and 3.14. No need to rerun expensive cache/TTL tests for documentation changes.
