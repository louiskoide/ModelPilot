...
Last updated: September 24, 2026. Read this first, then the relevant milestone guide/results before changing code.
...
- `bench_tools.py`: the ModelPilot arm's stdio MCP tools (`run_tests`, `search`, `expand_output`); host operations only, excerpt plus handle above the threshold. `fixture_dispatch.py`: fixture-only dispatcher and `ProxyPolicy` (ladder escalations in the proxy, owned fixture upstream only).
- `bench_tasks.py`: mines SWE-bench-style tasks from cloned repos (ignored `work/bench/repos/`), history-free checkouts, hidden grader (test dir restored to the reference commit, run without credentials, fresh HOME/TMPDIR, skips never count as run), validation (`python3 -m modelpilot.bench_tasks validate`). Task specs are in `bench/tasks/`. `bench.py`: task × arm × trial harness with isolation, identical limits, wire accounting and grading. It adds a $0 reference preflight (a trial must pass as many hidden tests as its reference), a `single` or `followup` shape (`--resume` after `--gap`, other trials run during the gap), a pinned client binary, session stop reasons, leftover-process cleanup, and crash-safe records. Fixed arms only for now; `--live` is billable and requires `--run-budget`. `bench_report.py`: cold-equivalent cost (cache reads inherited from earlier trials repriced as writes; measured cost alongside) and a paired task-level bootstrap; `python3 -m modelpilot.bench_report <run>` rebuilds a summary.
...
Proxy tests bind local loopback sockets; a sandbox denial is not a product test failure. Last verification: 312 offline tests on September 25, 2026 on Python 3.12 (`runs/bench-tools-regression.log`); the breakdown below is from the 244-test run (the earlier 198, plus 17 bench-report, 22 bench-harness, 5 bench-task and 2 proxy; the governed session and 5 offline bench trial tests drive the real `claude` CLI and are skipped in CI). On this Mac, system Python 3.9 passes 242, and the two `test_transport` HTTPError tests error because of a 3.9 `HTTPError` quirk, not a product failure. Homebrew Python 3.12 (`python3.12`) passes all 244. CI runs 3.10 and 3.14. No need to rerun expensive cache/TTL tests for documentation changes.
...
4. The ModelPilot arm's behavior is proposed in `docs/m6-modelpilot-policy.md`. It needs the user's approval for active mode in the benchmark arm only, and an M0 replication on Haiku 4.5, Sonnet 5 and Opus 5 first. Add integrated ModelPilot, stock pinned Jev, and fixed-model/effort arms. Record any model-constrained Jev variant separately. Measure all costs, quality, wall time, routing failures and uncertainty together. Existing direct-API toy results cannot be reused as a fair agent baseline.
