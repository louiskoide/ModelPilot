...
2. Integration surface. Done offline in `governor.py`: durable reserve/settle with crash recovery, ledger-fenced review, rebase plan/ack, journal, governed worker transport, and multi-process tests. Wiring done offline on branch `governor-wiring` (see `docs/governor.md`, "Wiring"): proxy settlement through shared `mp-` request IDs, Claude Code hooks (observe, correction delivery, user-only rebase and stuck notices), and gated fallback execution. The first live session (`runs/governed-session-20260922-214046`) failed: accounting was exact, but the model rejected the hook-delivered correction as prompt injection. The correction channel needs user-declared authority (see `docs/governor.md`, "Live evidence"). Remaining: that fix and a second live run, detection of `/model` and `/effort` rebuilds, and turn-end correction delivery. Keep dry-run until item 4 has evidence.
