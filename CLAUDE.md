...
3. Plan in `docs/m6-benchmark-plan.md` (tasks mined from real repositories, SWE-bench style, split by repository into tuning/final). 3a and 3b done offline: 5 validated tasks (more-itertools, tomli) and the fixed-arm harness. Next is the 3c live pilot. The local Python is 3.9 only, which limits the mined corpus. Build representative, reproducible repository-task fixtures and shared graders; isolate checkouts and normalize tool access, model availability, auth, cache state and effort. Separate tuning from final evaluation.
