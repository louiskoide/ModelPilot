...
Proxy tests bind local loopback sockets; a sandbox denial is not a product test failure. Last verification: 191 offline tests on September 22, 2026 (the earlier 143, plus 7 governed-proxy, 2 enforce/reconcile, 7 fallback, 2 cascade-harness, 17 hook, 1 offline governed-session, 7 bench-task and 5 bench-harness; the governed session and one bench trial test need the `claude` CLI and are skipped in CI). On this Mac only system Python 3.9 exists; there 189 pass and the two `test_transport` HTTPError tests error because of a 3.9 `HTTPError` quirk, not a product failure. CI runs 3.10 and 3.14. No need to rerun expensive cache/TTL tests for documentation changes.
...
3. Plan in `docs/m6-benchmark-plan.md` (tasks mined from real repositories, SWE-bench style, split by repository into tuning/final). 3a and 3b done offline: 5 validated tasks (more-itertools, tomli) and the fixed-arm harness. The 3c live pilot passed (`runs/bench-20260923-070044`). Next is 3d, the full corpus with harder tasks. The local Python is 3.9 only, which limits the mined corpus. Build representative, reproducible repository-task fixtures and shared graders; isolate checkouts and normalize tool access, model availability, auth, cache state and effort. Separate tuning from final evaluation.
